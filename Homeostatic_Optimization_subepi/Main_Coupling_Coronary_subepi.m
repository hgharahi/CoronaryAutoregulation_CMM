...
global H0 rho_w beta gamma R0 Rmin alpha_t Pin Pout N_gen Pim A LA IA SA Ghe1 Ghe2 Ghc Ghm S lmax lmin fiber_ang 
...
inputs = ReadInputs('subepicardium');
Pin = inputs(1)*133.32;    Pout = inputs(2)*133.32;	Pim = inputs(3)*133.32;

sheet = 'Params_subepi';
A = ReadParams('B',sheet); %small artery mechanical parameters   
LA = ReadParams('C',sheet); %large arteriole mechanical parameters   
IA = ReadParams('D',sheet); %intermediate arteriole mechanical parameters   
SA = ReadParams('E',sheet); %small arteriole mechanical parameters   
...
for kk=1:N_gen    
    ...
    Pmid(kk) = p_mid(kk);
    Thickness(kk) =(1/(SMCtoCOL(kk)+1)*Mt(kk) + ...
...
for k=1:N_gen
    ...
    % for output
    mu(k) = viscosity(2*Radius(k));
%     ratio(k) = Hu(k)/(2*Ru(k));
    ratio(k) = Thickness(k)/(2*Radius(k));
    sigma_h(k) = (Pmid(k)-Pim).*Radius(k)/(Thickness(k));       % homeostatic stress
    shear(k) = 4*mu(k)*qSteady(k)/(pi*Radius(k)^3);% homeostatic shear stress
end  
...
for k = 1:N_gen
    [KC, Act_lvl(k), Smax(k)] = mechanical_properties_CA(Pmid(k)-Pim,Radius(k));
    c1(k) = KC(1); c2(k) = KC(2); c3(k) = KC(3); c4(k) = KC(4); c5(k) = KC(5);
    Ge1(k) = Ghe1; Ge2(k) = Ghe2; 
    
    Gc(k) = Ghc;   Gm(k) =  Ghm;
%     Sbasal(k) = Smax(k);
    rhow(k) = rho_w;
    l_max(k) = lmax; l_min(k) = lmin;
    angle(k) = fiber_ang; 
...
close all;
save([newdir,'\OptimizationResults\subepi_2020']);
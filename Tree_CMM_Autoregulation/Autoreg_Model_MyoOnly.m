...
a = 3;
[q_network1d, A1d, R1d, SMAX1d, TAU1d, PMID1d, S1d] = Eval_K(K_est(4:6), subendo);
[q_network2d, A2d, R2d, SMAX2d, TAU2d, PMID2d, S2d] = Eval_K(K_est(4:6), subepi);

a = 4;
[q_network1g, A1g, R1g, SMAX1g, TAU1g, PMID1g, S1g] = Eval_K(K_est(4:6), subendo);
[q_network2g, A2g, R2g, SMAX2g, TAU2g, PMID2g, S2g] = Eval_K(K_est(4:6), subepi);

M = 100;
a = 1;
[q_network1e, A1e, R1e, SMAX1e, TAU1e, PMID1e, S1e] = Eval_K(K_est(4:6), subendo);
[q_network2e, A2e, R2e, SMAX2e, TAU2e, PMID2e, S2e] = Eval_K(K_est(4:6), subepi);


M = 140;
a = 1;
[q_network1f, A1f, R1f, SMAX1f, TAU1f, PMID1f, S1f] = Eval_K(K_est(4:6), subendo);
[q_network2f, A2f, R2f, SMAX2f, TAU2f, PMID2f, S2f] = Eval_K(K_est(4:6), subepi);


% M = 1;
% a = 1;
% [q_network1g, A1f, R1f, SMAX1f, TAU1f, PMID1f, S1f] = Eval_K(K_est(4:6), subendo);
% [q_network2g, A2f, R2f, SMAX2f, TAU2f, PMID2f, S2f] = Eval_K(K_est(4:6), subepi);
%% Plots
...
p4 = plot(p_in/133.32, (q_network2c + q_network1c)./(subepi.q(end)+subendo.q(end)),'-.b','LineWidth',1.5);
p5 = plot(p_in/133.32, (q_network2d + q_network1d)./(subepi.q(end)+subendo.q(end)),'-.r','LineWidth',1.5);
p8 = plot(p_in/133.32, (q_network2g + q_network1g)./(subepi.q(end)+subendo.q(end)),'-.g','LineWidth',1.5);
% p8 = plot(p_in/133.32, (q_network2g + q_network1g)./(subepi.q(end)+subendo.q(end)),'.g','LineWidth',1.5);

xlabel('p_{in} (mmHg)','FontSize',12); ylabel('$q/q_{h}$','Interpreter','latex','FontSize',12);
legend([p2, p3, p4, p5, p8],'All mechanisms', 'Fully dilated trees','Myogenic','Metabolic+Shear-dependent','Fully constricted','Location','Best','FontSize',12);
grid on;
box on;
ylim([0 3]);
...
p2 = plot(p_in/133.32, (q_network2 + q_network1)./(subepi.q(end)+subendo.q(end)),'-k','LineWidth',1.5);
p3 = plot(p_in/133.32, (q_network2p + q_network1p)./(subepi.q(end)+subendo.q(end)),'--k','LineWidth',1.5);
p6 = plot(p_in/133.32, (q_network2e + q_network1e)./(subepi.q(end)+subendo.q(end)),'-.b','LineWidth',1.5);
p7 = plot(p_in/133.32, (q_network2f + q_network1f)./(subepi.q(end)+subendo.q(end)),'-.r','LineWidth',1.5);
% p8 = plot(p_in/133.32, (q_network2g + q_network1g)./(subepi.q(end)+subendo.q(end)),'.g','LineWidth',1.5);

xlabel('p_{in} (mmHg)','FontSize',12); ylabel('$q/q_{h}$','Interpreter','latex','FontSize',12);
legend([p2, p3,  p6, p7],'All mechanisms', 'Fully dilated trees','Exercise 1 (1.3Q_{target})','Exercise 2 (2Q_{target})','Location','Best','FontSize',12);
grid on;
box on;

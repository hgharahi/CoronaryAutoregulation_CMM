function [q_network, A, R, SMAX, TAU, PMID, S] = L_NAME(K, tree)


global p_in  p_out  M a 
...
%         eps = -1; % a very large signal for metabolic, essentially making the activation zero.
%                 if eps > 0
%                     eps = 0;
%                 end
...
%             s_tau(i) = - phi_s(i) * (Ks *(tau(i) - tree.tau_h(i))/tree.tau_h(i));
            ...
            if strcmp(tree.ID(i),'B') || strcmp(tree.ID(i),'C')
            A(i,j) = 1;
            else
                A(i,j) = 0;
            end

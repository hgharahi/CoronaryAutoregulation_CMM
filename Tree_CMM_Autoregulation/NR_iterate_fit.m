function [r0, T_act] = NR_iterate_fit(Me,Mk,Mm,r,hm,P,x, r_act, rh, flg, subendo, i, ID, A)
...
Lt = r0/r_h;
%
if (abs(Lt-1) > 0.8 || isnan(Lt)) && flg == 0
    
%     P/133.32
    rp(1) = NR_interpolation(Me,Mk,Mm,r,hm,P - 3*133.32, r_act, rh, subendo, i, ID, A);
    rp(2) = NR_interpolation(Me,Mk,Mm,r,hm,P - 5*133.32, r_act, rh, subendo, i, ID, A);
    rp(3) = NR_interpolation(Me,Mk,Mm,r,hm,P + 3*133.32, r_act, rh, subendo, i, ID, A);
    rp(4) = NR_interpolation(Me,Mk,Mm,r,hm,P + 5*133.32, r_act, rh, subendo, i, ID, A);
    rp(5) = NR_interpolation(Me,Mk,Mm,r,hm,P - 7*133.32, r_act, rh, subendo, i, ID, A);
    rp(6) = NR_interpolation(Me,Mk,Mm,r,hm,P + 7*133.32, r_act, rh, subendo, i, ID, A);
    rp(7) = NR_interpolation(Me,Mk,Mm,r,hm,P - 9*133.32, r_act, rh, subendo, i, ID, A);
    rp(8) = NR_interpolation(Me,Mk,Mm,r,hm,P + 9*133.32, r_act, rh, subendo, i, ID, A);
    rp((rp./r_h)>1.3)=[];
    r0 = nanmean(rp(rp>0));
end

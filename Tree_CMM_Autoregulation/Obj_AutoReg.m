function Obj_AutoReg(tree, p_in, p_out, af, ap, am)
...
for j = 1:length(p_in)
    ...
    while er > 10^-6
        ...
        for i = 1:tree.N_gen
            ...
            if af == 1
                FF = shear_control(tau, tree.tau_h, tree.ID(i));
            else
                FF = 0.5;
            end
            
            if ap ~= 0
                PF = ap*myogenic_control(p_mid(i) - tree.Pim , tree.Pmid(i) - tree.Pim, tree.ID(i)); %% the  multiplier 0.5 here is to get the basal tone!
            else
                PF = 1;
            end
           ...
           if am == 1
            ...
            x = mechanical_params(tree, i, MF*FF*PF, tree.ID(i));
